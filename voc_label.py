...
def convert_annotation(year, image_id):
    in_file = open('train-image/%s/Annotations/%s.xml'%(year, image_id))
    out_file = open('train-image/%s/labels/%s.txt'%(year, image_id), 'w')
    ...
    if root.find('object')==None:           #
        ...
        # print(os_file_path+'/train-image/%s/Annotations/%s.xml'%(year,image_id))
...
for year, image_set in sets:
    if not os.path.exists('train-image/%s/labels/'%(year)):
        os.makedirs('train-image/%s/labels/'%(year))
    #image_ids = open('train-image/%s/ImageSets/Main/%s.txt'%(year, image_set)).read().strip().split()
    image_ids = open('train-image/%s/ImageSets/Main/%s.txt'%(year, image_set)).read().strip().split()
    list_file = open('%s_%s.txt'%(year, image_set), 'w')
    for image_id in image_ids:
        list_file.write('%s/train-image/%s/JPEGImages/%s.jpg\n'%(os_file_path.replace('\\','/'), year, image_id))
